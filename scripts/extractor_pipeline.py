...
from multiple_face_detect import FaceDetection
...
class ExtractorPipeline():
    ...
    def get_txt(self):
        """
        extract text from all preview images in the assets folder
        """
        ...
        for folder in folder_list:
            ...
            # check if files exist
            if os.path.exists(query_img):
                text = text_detect_class.extract_text(query_img)
                extracted_texts.append([folder.split('/')[-1], text])
            else:
                # files not found
                extracted_texts.append([folder.split('/')[-1], ""])

        # save as dataframe
        df = pd.DataFrame(extracted_texts, columns=['id', 'asset_text'])
        # save as CSV file
        df.to_csv(self.extracted_path + '/asset_texts.csv', index=False)

    def get_dominant_colours(self):
        """
        each preview image in an asset folder has a wide variety of colours.

        saving image colours as a separate file
        """
        # create colours folder inside extracted features folder
        colours_path = self.extracted_path+'/colours'
        ...
        for folder in folder_list:
            ...
            # check if file exists
            if os.path.exists(query_img):
                # dataframe object that contains all colours extracted from the image
